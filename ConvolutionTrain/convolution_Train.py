...
# 第二层卷积层
model.add(Conv2D(filters=64, kernel_size=3,strides=1 , padding='same'))#, kernel_regularizer=regularizers.l2(0.01)))
#model.add(BatchNormalization())
model.add(Activation('relu'))
...
# 第三层卷积层
model.add(Conv2D(filters=128, kernel_size=3,strides=1 , padding='same'))#, kernel_regularizer=regularizers.l2(0.01)))
#model.add(BatchNormalization())
model.add(Activation('relu'))
model.add(Conv2D(filters=128, kernel_size=3,strides=1 , padding='same'))#, kernel_regularizer=regularizers.l2(0.01)))
#model.add(BatchNormalization())
...
model.save('my_base_model_relu.h5')

from paint_Process import paint_process

name = "my_base_model_relu"
